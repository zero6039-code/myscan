...
function shareOnPlatform(platform) {
    ...
    const title = encodeURIComponent('NetEye Security Report');
    // 添加友情提示
    const disclaimer = encodeURIComponent('\n\n⚠️ This report is for authorized security testing only. Unauthorized scanning is prohibited.\nGenerated by NetEye Scanner | Contact: zero6039@gmail.com');
    let shareUrl = '';
    switch (platform) {
        case 'whatsapp':
            shareUrl = `https://wa.me/?text=${title}%20${url}${disclaimer}`;
            break;
        case 'facebook':
        ...
        case 'messenger':
            ...
            break;
        case 'email':
            shareUrl = `mailto:?subject=${title}&body=${url}${decodeURIComponent(disclaimer)}`;
            break;
        case 'wechat':
            navigator.clipboard.writeText(window.location.href + '\n\n' + decodeURIComponent(disclaimer));
            alert('Link copied to clipboard. Open WeChat and paste to share.');
            return;
        case 'line':
            shareUrl = `https://line.me/R/msg/text/?${title}%20${url}${disclaimer}`;
            break;
        default:
            return;
    }
